...
* **Storage refuses a destination that is not a plain filename.** The name is reduced to a `basename()`, and `''`, `.`, `..`, a leading dot and names containing a null byte are rejected. `resolveFilename()`, the protected naming seam, derives the name; `upload()` decides whether it may be written, so **an override of the seam cannot drop any of these refusals**. With the refusals inside the seam, `return basename($fileInfo->getNameWithExtension())` — the obvious override to write — stored every one of them. A leading dot matters most: the deny-list carries `htaccess`, but a dotfile presents no extension for it to match, so `.htaccess` reached the upload directory
