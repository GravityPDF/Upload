...
* **The filename the library puts in an error string is sanitized.** Raw `$_FILES[…]['name']` reached `getErrors()` from the constructor, and a name from a custom `FileInfoInterface` from `isValid()`, so a line break, terminal escape or bidi override could land in a string the README tells you to render. The shipped `FileInfo` was unaffected. A validator's own message is still reported as thrown, so a custom `FileInfoInterface` returning an unsanitized extension can still reach `getErrors()` through `Validation\FileType`. **Escape on output regardless**
